

# data models: image

manage images



## examples

### DELETE

remove image (logo, album, profile, etc.)



#### example

merchant delete own logo



##### request

```json
{
  "httpMethod": "DELETE",
  "path": "/image",
  "requestContext": {
    "authorizer": {
      "claims": {
        "sub": "c272e5c2-ea69-4605-9489-816624ed731b",
        "cognito:username": "merchant",
        "cognito:groups": "merchant"
      }
    }
  },
  "queryStringParameters": {},
  "body": {
    "url": "https://img.favr.town/content/logo/WFo4DCS4b.png"
  }
}
```



##### response

```json
{
  "timeStamp": "1626801171694",
  "item": {
    "flag_updated": 1626801171
  }
}
```


#### example

merchant remove  own reward image



##### request

```json
{
  "httpMethod": "DELETE",
  "path": "/image",
  "requestContext": {
    "authorizer": {
      "claims": {
        "sub": "c272e5c2-ea69-4605-9489-816624ed731b",
        "cognito:username": "merchant",
        "cognito:groups": "merchant"
      }
    }
  },
  "queryStringParameters": {},
  "body": {
    "url": "https://img.favr.town/content/reward/DQXXJHXxf.png",
    "sk": "H5C-wCPHA"
  }
}
```



##### response

```json
{
  "timeStamp": "1634066864682",
  "item": {
    "flag_updated": 1634066865,
    "rule_rewards": [
      {
        "favr": 2000,
        "sk": "H5C-wCPHA",
        "description": "Test 101",
        "redeem_by": 1633086900,
        "flag_updated": 1634060893
      },
      {
        "favr": 2000,
        "sk": "pF3RWgnmD",
        "redeem_by": 1634642100,
        "description": "Buy one, get one free"
      }
    ]
  }
}
```



### PATCH

add image (logo, album, profile, etc.)



#### example

merchant add logo



##### request

```json
{
  "httpMethod": "PATCH",
  "path": "/image",
  "requestContext": {
    "authorizer": {
      "claims": {
        "sub": "c272e5c2-ea69-4605-9489-816624ed731b",
        "cognito:username": "merchant",
        "cognito:groups": "merchant"
      }
    }
  },
  "queryStringParameters": {},
  "body": {
    "image": "",
    "type": "logo"
  }
}
```



##### response

```json
{
  "timeStamp": "1626801483384",
  "item": {
    "flag_updated": 1626801483,
    "logo": "https://img.favr.town/content/logo/-6AZrBmGK.png"
  }
}
```



#### example

merchant add image to reward by type, sk



##### request

```json
{
  "httpMethod": "PATCH",
  "path": "/image",
  "requestContext": {
    "authorizer": {
      "claims": {
        "sub": "c272e5c2-ea69-4605-9489-816624ed731b",
        "cognito:username": "merchant",
        "cognito:groups": "merchant"
      }
    }
  },
  "queryStringParameters": {},
  "body": {
    "image": "(image string)",
    "type": "reward",
    "sk": "H5C-wCPHA"
  }
}
```



##### response

```json
{
  "timeStamp": "1634066398570",
  "item": {
    "flag_updated": 1634066398,
    "rule_rewards": [
      {
        "favr": 2000,
        "sk": "H5C-wCPHA",
        "description": "Test 101",
        "redeem_by": 1633086900,
        "image": "https://img.favr.town/content/reward/cRb26UAzK.png",
        "flag_updated": 1634060893
      },
      {
        "favr": 2000,
        "sk": "pF3RWgnmD",
        "redeem_by": 1634642100,
        "description": "Buy one, get one free"
      }
    ]
  }
}
```



## event definition

```json
{
  "event": {
    "DELETE": {
      "type": "object",
      "properties": {
        "httpMethod": {
          "type": "string"
        },
        "params": {
          "type": "object",
          "properties": {
            "model": {
              "type": "string"
            },
            "requestor": {
              "type": "string"
            },
            "login": {
              "type": "string"
            },
            "role": {
              "oneOf": [
                {
                  "type": "string",
                  "pattern": "^merchant$"
                },
                {
                  "type": "string",
                  "pattern": "^patron$"
                }
              ]
            }
          },
          "required": ["model", "requestor", "login", "role"]
        },
        "body": {
          "type": "object",
          "properties": {
            "url": {
              "type": "string"
            },
            "sk": {
              "type": "string"
            }
          },
          "required": ["url"]
        }
      },
      "required": ["httpMethod", "params", "body"]
    },
    "PATCH": {
      "type": "object",
      "properties": {
        "httpMethod": {
          "type": "string"
        },
        "params": {
          "type": "object",
          "properties": {
            "model": {
              "type": "string"
            },
            "requestor": {
              "type": "string"
            },
            "login": {
              "type": "string"
            },
            "role": {
              "oneOf": [
                {
                  "type": "string",
                  "pattern": "^merchant$"
                },
                {
                  "type": "string",
                  "pattern": "^patron$"
                }
              ]
            }
          },
          "required": ["model", "requestor", "login", "role"]
        },
        "body": {
          "type": "object",
          "properties": {
            "image": {
              "type": "string"
            },
            "image_text": {
              "type": "string"
            },
            "sk": {
              "type": "string"
            },
            "type": {
              "oneOf": [
                {
                  "type": "string",
                  "pattern": "^logo$"
                },
                {
                  "type": "string",
                  "pattern": "^album$"
                },
                {
                  "type": "string",
                  "pattern": "^profile$"
                },
                {
                  "type": "string",
                  "pattern": "^reward$"
                },
                {
                  "type": "string",
                  "pattern": "^news$"
                },
                {
                  "type": "string",
                  "pattern": "^survey$"
                }
              ]
            }
          },
          "required": ["image", "type"]
        }
      },
      "required": ["httpMethod", "params", "body"]
    }
  }
}
```

